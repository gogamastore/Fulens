...
class BotEngine:
    ...
    def _handle_symbol(self, symbol: str, all_pos: list, atr_map: dict):
        decision = fulens_client.fetch_signal(symbol)
        if decision is None:
        ...
        # 3) Buka posisi baru searah bila memenuhi gerbang & kapasitas.
        same_dir = [p for p in sym_pos if p["type"] == target]
        total_open = len(all_pos) - closed
        capacity = min(self.s.max_positions_per_symbol - len(same_dir),
                       self.s.max_open_positions - total_open)
        rec["planned_entries"] = max(0, capacity) if actionable else 0

        if actionable and capacity > 0:
            ...
            if plan and plan.volume > 0:
                ...
                if acc and need is not None and acc.get("free_margin", 0) >= need:
                    result = self.executor.open_market(
                        ...
                        comment=f"fulens {decision.raw_signal}"[:26])
                    if result["ok"]:
                        rec["executed"] = True
