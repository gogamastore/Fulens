...
def fetch_signal(symbol: str, timeframe: str = "D1") -> SignalDecision | None:
    """Ambil sinyal FuLens untuk `symbol` pada `timeframe` → keputusan arah.

    FuLens (symbols.normalize) mengenali nama broker standar + suffix umum.
    Bila simbol tak dikenali otak, endpoint balas 4xx → fungsi ini return None.
    """
